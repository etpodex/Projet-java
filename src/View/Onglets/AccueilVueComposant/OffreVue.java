...
public class OffreVue extends JPanel {
    ...
    private int nombre_de_panel_offre = 3; // Nombre de panneaux correspondant au nombre d'offres
    private double[] poids_panels = {0.33, 0.34, 0.33}; // Poids des panneaux ajusté pour trois offres
    ...
    private void creerEtAjouterPanels(GridBagConstraints gbc, int hauteur) {
        for (int i = 0; i < nombre_de_panel_offre; i++) {
            ...
            String texteOffre = String.format("Avec le code %s obtenez %d%% de réduction", offres[i].getCode_promo(), offres[i].getReduction());
